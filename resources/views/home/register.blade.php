            ...
            <div class="card-body p-5">
              ...
              <form action="{{ route("post-register") }}" method="POST">
                ...
                <div data-mdb-input-init class="form-outline mb-3">
                  @error("name")<span class="text-danger ml-2 custom-text-invalid ">{{ $message }}</span>@enderror
                  <input  value="{{ $errors->has('name') ? '' : old('name')}}" type="text" id="form3Example1cg" class="form-control form-control-lg {{ $errors->has('name') ? 'is-invalid' : (old('name') ? 'is-valid' : '') }}" name="name" placeholder="name" required/>
                ...
                <div data-mdb-input-init class="form-outline mb-3">
                  @error("confirm")<span class="text-danger ml-2 custom-text-invalid ">{{ $message }}</span>@enderror
                  <input type="password" id="form3Example4cdg" class="form-control form-control-lg @error('confirm') is-invalid @enderror" name="password_confirmation" placeholder="Confirm Password" required/>
                  <label class="form-label" for="form3Example4cdg">Repeat your password</label>
                </div>

                <div class="d-flex justify-content-center">
                  <button  type="submit" 
                    data-mdb-ripple-init class="btn btn-primary btn-block btn-lg gradient-custom-4 text-white">Register</button>
                </div>

                <p class="text-center text-muted mt-5 mb-0">Have already an account? <a href="{{ route("login") }}"
                    class="fw-bold text-danger"><u>Login here</u></a></p>
